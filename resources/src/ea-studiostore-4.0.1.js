...
(function(factory){
	...
	EASearchDemo.prototype = {
			defaults: {
				navTemplate: "../StudioStore/src/templates/navigation.hbs",
				prodTemplate: "../StudioStore/src/templates/products.hbs",
				server: '',
				dct: 'EcomDemo',
				idInput: 'question',
				idButton: 'search',
...
//						}
				...
				if ('onpopstate' in window){
					...
					window.onpopstate = function(event){
						if (event.state){
							...
							history.back();
							window.console && console.log("popstate nothing");
						}
						return false;
					};
				}
			...
			showDetails: function(id,item, path){
				if (item){
					var fields = this.options.fields;
					var dlg = $('#ea-detailed-page');
					$(dlg).find('img.ea-detailed-image').removeClass('image-not-found').one('error',function(){
						$(this).addClass('image-not-found');
					}).attr('src',this.value(item,fields.image) ||'');
					;
					$(dlg).find('.ea-detailed-desc').html(this.value(item,fields.desc));
					$(dlg).find('.ea-detailed-price').html(this.value(item,fields.price) || '&nbsp;');
					$(dlg).find('.ea-detailed-id').text(this.value(item,fields.id));
					$(dlg).dialog({
						dialogClass: 'ea-detailed-dialog',
						title: this.value(item,fields.name) + (this.value(item,fields.mfr)?(' by ' + this.value(item,fields.mfr)):''),
						width: 600,
						resizable: false,
						modal: true,
						buttons: [{
					...
					var result = [];
					this.getPromotionByType(id,0,this.promotionTypes,result);
				}
			},
			...
			insertProducts: function(prods){
				if (prods){
					...
					var fields = this.options.fields;
//4.0.1 Mod begin
					// normalize all the values by invoking value on the fields that are defined for store
					if (prods.groups){
							...
							for(var j = 0; j < group.item.length; j++){
								for(var field in fields){
									if (Object.prototype.hasOwnProperty.call(fields, field)){
										...
										group.item[j][fieldName] = this.value(group.item[j],fieldName);
									}
								}
							}
						}
						...
						for(var i = 0; i < prods.items.length; i++){
							for(var field in fields){
								if (Object.prototype.hasOwnProperty.call(fields, field)){
									var fieldName = fields[field];
									prods.items[i][fieldName] = this.value(prods.items[i],fieldName);
								}
							}
						}
					}
