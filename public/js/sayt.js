    			...
    			if (section.type.toLowerCase() == 'category'){
    				...
    				if (categories.length){
    					...
    					for(var i = 0; i < categories.length && i < section.size; i++){
    						result.vals.push({name: categories[i].name,
...
//    			window.console && console.log('generate suggestions on: "' + val +'"');
    		...
    		search: function(q){
    			...
    			var def = $.Deferred();
				$.ajax({
					url: self.config.serverSearch + self.config.urlSearch + '?' + self.config.defaultSearchParams + '&customer=easayt',
					dataType: 'jsonp',
					data: {
						dct: this.config.dict,
						q: q,
						ResultsPerPage: self.config.products.size
					},
